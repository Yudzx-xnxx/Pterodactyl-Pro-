...
class ServerTransferController extends Controller
{
    ...
    private function authorizeTransfer(): void
    {
        ...
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak! Transfer server hanya boleh dilakukan oleh Admin ID 1. 
©Protect By YudzxTech | t.me/Yudzxcysec');
        }
    }

    /**
     * Memulai proses transfer server ke node baru.
     *
     * @throws \Throwable
     */
    ...
    /**
     * Menandai allocation sebagai milik server selama proses transfer.
     */
