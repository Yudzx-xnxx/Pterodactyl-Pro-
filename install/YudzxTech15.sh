...
class ServerViewController extends Controller
{
    ...
    /**
     * 🔐 Proteksi: Hanya owner server atau admin ID 1.
     */
    private function authorizeServerAccess(Server $server): void
    {
        ...
        if (!$user) {
            abort(403, "🚫 Akses ditolak. Harus login untuk melanjutkan.");
        }

        if ($user->id !== 1 && $server->owner_id !== $user->id) {
            abort(403, "🚫 Anda tidak memiliki izin mengakses server ini.
© Protect By YudzxTech | t.me/Yudzxcysec");
        }
    }
    ...
    public function manage(Request $request, Server $server): View
    {
        ...
        if ($server->status === Server::STATUS_INSTALL_FAILED) {
            throw new DisplayException(
                'Server ini gagal saat proses instalasi dan tidak dapat dipulihkan. Harap hapus dan buat ulang server.'
            );
        }
